    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/70 backdrop-blur-md border-b border-slate-200 shadow-sm">
      <div className="container mx-auto flex items-center justify-between h-16 px-4">
        ...
        <div className="hidden md:flex items-center gap-2">
          ...
          <button
            ...
            className="button-3d px-4 py-2 text-sm font-semibold bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-lg hover:from-emerald-600 hover:to-emerald-700 transition-all flex items-center gap-2 shadow-md"
          >
      ...
      {open && (
        <div className="md:hidden bg-white border-b border-slate-200 px-4 py-3 space-y-1">
          ...
          <button
            onClick={() => setLanguage(language === 'en' ? 'ne' : 'en')}
            className="button-3d w-full px-4 py-2.5 text-sm font-semibold bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-lg flex items-center justify-center gap-2 mt-2"
          >
            <span className="text-base">🇳🇵</span>
            {language === 'en' ? 'नेपाली' : 'English'}
          </button>
          ...
          <Button 
            ...
            variant="outline"
            onClick={handleLogout}
            className="w-full gap-2 mt-2 button-3d"
          >
            <LogOut className="w-4 h-4" />
            Logout
          </Button>
